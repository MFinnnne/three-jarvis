...
import {ParamsParser, ParamsParsers, parseParams} from '../../common/params-parsers';
 ...
 * @return A constraint or null if not found.
 */
export function createRangeConstraint(params: {max?: number; min?: number}): Constraint<number> | null {
 ...
 */
export function findNumberRange(c: Constraint<number>): [number | undefined, number | undefined] {
 ...
 * @hidden
 */
export const NumberInputPlugin: InputBindingPlugin<number, number, NumberInputParams> = {
	id: 'input-number',
	type: 'input',
	accept: (value, params) => {
	...
	controller: (args) => {
		...
		const formatter = ('format' in args.params ? args.params.format : undefined) ?? createNumberFormatter(getSuitableDecimalDigits(c, value.rawValue));
