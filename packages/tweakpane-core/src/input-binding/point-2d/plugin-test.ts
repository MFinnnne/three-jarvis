	...
	[
		...
		context(`when params = ${JSON.stringify(testCase.params)}`, () => {
			it(`should return ${testCase.expected}`, () => {
				const mv = getSuitableMaxValue(testCase.params.rawValue, testCase.params.constraint);
...
describe(Point2dInputPlugin.id, () => {
	it('should create appropriate step constraint', () => {
		...
		const constraint = (c?.binding.value as BoundValue<unknown>).constraint as PointNdConstraint<Point2d>;
