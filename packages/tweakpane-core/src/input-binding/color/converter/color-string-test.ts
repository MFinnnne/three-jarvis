...
import {ColorComponents3, ColorComponents4, ColorMode} from '../model/color-model';
import {
...
describe(createColorStringParser.name, () => {
		...
		testCase.inputs.forEach((input) => {
			context(`when ${JSON.stringify(input)}`, () => {
				it(`it should parse as ${JSON.stringify(testCase.expected)}`, () => {
					...
					assert.deepStrictEqual(actual && actual.toRgbaObject(), testCase.expected);
				});
			});
		});
	});
	...
	[
		...
		context(`when ${JSON.stringify(testCase.input)}`, () => {
			it(`it should detect notation as ${JSON.stringify(testCase.expected)}`, () => {
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert string to color', () => {
				assert.deepStrictEqual(createColorStringBindingReader('int')(testCase.input).toRgbaObject(), testCase.expected);
			});
		});
	});
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert color to string', () => {
				assert.strictEqual(colorToHexRgbString(testCase.input), testCase.expected);
			});
		});
	});
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert color to int RGBA string', () => {
				assert.strictEqual(colorToFunctionalRgbaString(testCase.input), testCase.frgba);
			});
		});
	});
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert color to RGB string', () => {
				assert.strictEqual(colorToFunctionalRgbString(testCase.input), testCase.expected.int);
				assert.strictEqual(colorToFunctionalRgbString(testCase.input, 'float'), testCase.expected.float);
			});
		});
	});
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert color to float RGBA string', () => {
				assert.strictEqual(colorToFunctionalRgbaString(testCase.input, 'float'), testCase.expected);
			});
		});
	});
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert color to RGB object', () => {
				assert.strictEqual(colorToObjectRgbString(testCase.input, 'int'), testCase.expected.int);
				assert.strictEqual(colorToObjectRgbString(testCase.input, 'float'), testCase.expected.float);
			});
		});
	});
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert color to HSL string', () => {
				assert.strictEqual(colorToFunctionalHslString(testCase.input), testCase.expected.hsl);
				assert.strictEqual(colorToFunctionalHslaString(testCase.input), testCase.expected.hsla);
			});
		});
	});
	...
	(
		...
		context(`when ${JSON.stringify(testCase.components)}`, () => {
			it(`it should format to ${JSON.stringify(testCase.hex)}`, () => {
				...
				const f = testCase.components.length === 3 ? colorToHexRgbString : colorToHexRgbaString;
