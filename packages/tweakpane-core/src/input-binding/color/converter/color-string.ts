...
import {ColorComponents3, ColorComponents4, ColorMode, ColorType, removeAlphaComponent} from '../model/color-model';
...
function equalsStringColorFormat(f1: StringColorFormat, f2: StringColorFormat): boolean {
...
function parseFunctionalRgbColorComponents(text: string): ColorComponents3 | null {
	const m = text.match(/^rgb\(\s*([0-9A-Fa-f.]+%?)\s*,\s*([0-9A-Fa-f.]+%?)\s*,\s*([0-9A-Fa-f.]+%?)\s*\)$/);
	if (!m) {
		return null;
	}

	const comps: ColorComponents3 = [parseCssNumberOrPercentage(m[1], 255), parseCssNumberOrPercentage(m[2], 255), parseCssNumberOrPercentage(m[3], 255)];
...
function parseFunctionalRgbaColorComponents(text: string): ColorComponents4 | null {
	const m = text.match(/^rgba\(\s*([0-9A-Fa-f.]+%?)\s*,\s*([0-9A-Fa-f.]+%?)\s*,\s*([0-9A-Fa-f.]+%?)\s*,\s*([0-9A-Fa-f.]+%?)\s*\)$/);
	if (!m) {
		return null;
	}

	const comps: ColorComponents4 = [parseCssNumberOrPercentage(m[1], 255), parseCssNumberOrPercentage(m[2], 255), parseCssNumberOrPercentage(m[3], 255), parseCssNumberOrPercentage(m[4], 1)];
...
function parseHslColorComponents(text: string): ColorComponents3 | null {
	...
	const comps: ColorComponents3 = [parseCssNumberOrAngle(m[1]), parseCssNumberOrPercentage(m[2], 100), parseCssNumberOrPercentage(m[3], 100)];
	if (isNaN(comps[0]) || isNaN(comps[1]) || isNaN(comps[2])) {
		return null;
	}
	return comps;
...
function parseHslaColorComponents(text: string): ColorComponents4 | null {
	...
	const comps: ColorComponents4 = [parseCssNumberOrAngle(m[1]), parseCssNumberOrPercentage(m[2], 100), parseCssNumberOrPercentage(m[3], 100), parseCssNumberOrPercentage(m[4], 1)];
	if (isNaN(comps[0]) || isNaN(comps[1]) || isNaN(comps[2]) || isNaN(comps[3])) {
		return null;
	}
	return comps;
...
function parseObjectRgbColorComponents(text: string): ColorComponents3 | null {
	...
	const comps: ColorComponents3 = [parseFloat(m[1]), parseFloat(m[2]), parseFloat(m[3])];
	if (isNaN(comps[0]) || isNaN(comps[1]) || isNaN(comps[2])) {
		return null;
	}
	return comps;
...
function parseObjectRgbaColorComponents(text: string): ColorComponents4 | null {
	...
	const comps: ColorComponents4 = [parseFloat(m[1]), parseFloat(m[2]), parseFloat(m[3]), parseFloat(m[4])];
	if (isNaN(comps[0]) || isNaN(comps[1]) || isNaN(comps[2]) || isNaN(comps[3])) {
		return null;
	}
	return comps;
...
export function getColorNotation(text: string): StringColorNotation | null {
	const notations = Object.keys(NOTATION_TO_PARSER_MAP) as StringColorNotation[];
...
function detectStringColor(text: string): DetectionResult | null {
	return PARSER_AND_RESULT.reduce((prev: DetectionResult | null, {parser, result: detection}) => {
		if (prev) {
			return prev;
		}
...
export function detectStringColorFormat(text: string, type: ColorType = 'int'): StringColorFormat | null {
 ...
 * @hidden
 */
export const CompositeColorParser: Parser<Color> = (text: string): Color | null => {
...
export function createColorStringBindingReader(type: ColorType): BindingReader<Color> {
 ...
 * @hidden
 */
export function colorToFunctionalRgbString(value: Color, opt_type?: ColorType): string {
 ...
 * @hidden
 */
export function colorToFunctionalRgbaString(value: Color, opt_type?: ColorType): string {
 ...
 * @deprecated
 */
export function getColorStringifier(notation: StringColorNotation): (value: Color) => string {
...
const FORMAT_AND_STRINGIFIERS: FormatAndStringifier[] = [
	...
	...(['int', 'float'] as ColorType[]).reduce((prev: FormatAndStringifier[], type) => {
		return [
			...prev,
			{
				format: {
...
export function findColorStringifier(format: StringColorFormat): Formatter<Color> | null {
	return FORMAT_AND_STRINGIFIERS.reduce((prev: Formatter<Color> | null, fas) => {
		if (prev) {
			return prev;
		}
