...
import {colorFromObject, colorFromRgbaNumber, colorFromRgbNumber, colorToRgbaNumber, colorToRgbNumber, numberToRgbColor} from './color-number';
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert object to color', () => {
				assert.deepStrictEqual(colorFromObject(testCase.input).toRgbaObject(), testCase.expected);
			});
		});
	});
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert number to rgb color', () => {
				assert.deepStrictEqual(colorFromRgbNumber(testCase.input).toRgbaObject(), testCase.expected);
			});
		});
	});
	...
	[
		...
		context(`when input = ${JSON.stringify(testCase.input)}`, () => {
			it('should convert number to rgba color', () => {
				assert.deepStrictEqual(colorFromRgbaNumber(testCase.input).toRgbaObject(), testCase.expected);
			});
		});
	});
});
