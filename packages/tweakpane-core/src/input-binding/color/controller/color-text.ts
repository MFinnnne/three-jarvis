...
import {appendAlphaComponent, ColorMode, ColorType, getColorMaxComponents, removeAlphaComponent} from '../model/color-model';
...
export class ColorTextController implements Controller<ColorTextView> {
	...
	private createComponentControllers_(doc: Document): [NumberTextController, NumberTextController, NumberTextController] {
		const cc = {
		...
		const ccs: Tuple3<NumberTextController> = [createComponentController(doc, cc, 0), createComponentController(doc, cc, 1), createComponentController(doc, cc, 2)];
