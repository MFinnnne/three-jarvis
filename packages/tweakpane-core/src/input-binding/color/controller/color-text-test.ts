	...
	[
		...
		context(`when params = ${JSON.stringify(testCase.params)}`, () => {
			it(`should change component values to ${JSON.stringify(testCase.expected)}`, (done) => {
				const value = createValue(new Color(testCase.params.components as ColorComponents3, 'rgb'));
				value.emitter.on('change', () => {
					assert.deepStrictEqual(value.rawValue.toRgbaObject(), testCase.expected);
					done();
				});
