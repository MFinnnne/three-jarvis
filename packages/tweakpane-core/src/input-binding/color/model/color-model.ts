 ...
 * @hidden
 */
export function removeAlphaComponent(comps: ColorComponents4): ColorComponents3 {
 ...
 * @hidden
 */
export function appendAlphaComponent(comps: ColorComponents3, alpha: number): ColorComponents4 {
	return [comps[0], comps[1], comps[2], alpha];
}

const MODE_CONVERTER_MAP: {
	[fromMode in ColorMode]: {
		[toMode in ColorMode]: (c1: number, c2: number, c3: number) => ColorComponents3;
	};
} = {
	hsl: {
		hsl: (h, s, l) => [h, s, l],
 ...
 * @hidden
 */
export function getColorMaxComponents(mode: ColorMode, type: ColorType): ColorComponents3 {
 ...
 * @hidden
 */
export function constrainColorComponents(components: ColorComponents3 | ColorComponents4, mode: ColorMode, type: ColorType): ColorComponents4 {
...
function convertColorType(comps: ColorComponents3, mode: ColorMode, from: ColorType, to: ColorType): ColorComponents3 {
	const fms = getColorMaxComponents(mode, from);
	const tms = getColorMaxComponents(mode, to);
	return comps.map((c, index) => (c / fms[index]) * tms[index]) as ColorComponents3;
}

/**
 * @hidden
 */
export function convertColor(components: ColorComponents3, from: {mode: ColorMode; type: ColorType}, to: {mode: ColorMode; type: ColorType}): ColorComponents3 {
