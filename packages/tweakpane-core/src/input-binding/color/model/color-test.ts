...
describe(Color.name, () => {
	(
		...
		context(`when ${JSON.stringify(params)}`, () => {
			...
			it('should get components', () => {
				assert.deepStrictEqual(c.getComponents('rgb'), [expected.r, expected.g, expected.b, expected.a]);
			});
	...
	[null, undefined, {}, {r: 0}, {g: 127}, {b: 255}, {r: 0, g: 127}, {g: 0, b: 255}, {r: 0, b: 255}, {r: 0, g: 127, b: null}, {r: 0, g: '127', b: 255}].forEach((input: unknown) => {
		context(`when ${JSON.stringify(input)}`, () => {
	...
	[
		...
		context(`when ${JSON.stringify(params)}`, () => {
			it('should get components with specific mode', () => {
				const c = new Color(params.components as ColorComponents4, params.fromMode as ColorMode);
	...
	[
		...
		context(`when ${JSON.stringify(params)}`, () => {
			it('should constrain components', () => {
				const c = new Color(params.components as ColorComponents4, params.mode as ColorMode, params.type as ColorType);
				const o = c.getComponents(params.mode as ColorMode, params.type as ColorType);
				expected.forEach((e, index) => {
					assert.ok(TestUtil.closeTo(o[index], e, DELTA), `components[${index}]`);
				});
			});
		});
	});
	...
	(
		...
		context(`when ${JSON.stringify(params)}`, () => {
			it(`it should constrain hue range`, () => {
				...
				assert.ok(TestUtil.closeTo(c1.getComponents('hsl', params.type)[0], expected, DELTA));

				const c2 = new Color([params.hue, 0, 0], 'hsv', params.type);
				assert.ok(TestUtil.closeTo(c2.getComponents('hsv', params.type)[0], expected, DELTA));
			});
		});
	});
});
