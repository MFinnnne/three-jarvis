...
import {colorToObjectRgbaString, colorToObjectRgbString, createColorStringParser} from './converter/color-string';
...
import {ColorInputParams, extractColorType, parseColorInputParams} from './util';

function shouldSupportAlpha(initialValue: RgbColorObject | RgbaColorObject): boolean {
	return Color.isRgbaColorObject(initialValue);
}

function createColorObjectReader(opt_type: ColorType | undefined): BindingReader<Color> {
	return (value: unknown): Color => {
		return colorFromObject(value, opt_type);
	};
}

function createColorObjectFormatter(supportsAlpha: boolean, type: ColorType): Formatter<Color> {
	return (value) => {
		if (supportsAlpha) {
 ...
 * @hidden
 */
export const ObjectColorInputPlugin: InputBindingPlugin<Color, RgbColorObject | RgbaColorObject, ColorInputParams> = {
