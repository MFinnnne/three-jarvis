...
import {colorFromRgbaNumber, colorFromRgbNumber} from './converter/color-number';
import {colorToHexRgbaString, colorToHexRgbString, createColorStringParser} from './converter/color-string';
...
function createFormatter(supportsAlpha: boolean): Formatter<Color> {
	return supportsAlpha ? (v: Color) => colorToHexRgbaString(v, '0x') : (v: Color) => colorToHexRgbString(v, '0x');
}
 ...
 * @hidden
 */
export const NumberColorInputPlugin: InputBindingPlugin<Color, number, ColorInputParams> = {
