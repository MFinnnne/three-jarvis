...
import {createEmptyBladeController, createEmptyLabelableController, createLabelController} from '../test-util';
...
describe(ButtonBladePlugin.id, () => {
	...
	[(doc: Document) => createEmptyBladeController(doc), (doc: Document) => createLabelController(doc, createEmptyLabelableController(doc))].forEach((createController) => {
		it('should not create API', () => {
			const doc = createTestWindow().document;
	...
	it('should apply initial params', () => {
		...
		assert.strictEqual(api.controller_.view.element.innerHTML.includes('initiallabel'), true);
	});
});
