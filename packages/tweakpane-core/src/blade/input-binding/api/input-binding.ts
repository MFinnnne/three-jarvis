 ...
 */
export class InputBindingApi<In, Ex> extends BladeApi<InputBindingController<In>> {
	...
	public on<EventName extends keyof InputBindingApiEvents<Ex>>(eventName: EventName, handler: (ev: InputBindingApiEvents<Ex>[EventName]['event']) => void): InputBindingApi<In, Ex> {
	 ...
	 */
	public setValue(rawValue: In, emit = true): void {
	...
	private onBindingChange_(ev: InputBindingEvents<In>['change']) {
		...
		if (ev.options.emit) {
			this.emitter_.emit('change', {
				event: new TpChangeEvent(this, forceCast(value), this.controller_.binding.target.presetKey, ev.options.last, ev.options.before),
			});
		}
	}
}
