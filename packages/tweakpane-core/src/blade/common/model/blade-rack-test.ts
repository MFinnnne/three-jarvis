...
function createInputBindingController(doc: Document): InputBindingController<boolean> {
...
function createMonitorBindingController(doc: Document): MonitorBindingController<string> {
...
function createValueBladeController(doc: Document): ValueBladeController<number, LabelView> {
...
describe(BladeRack.name, () => {
	...
	it('should update positions', () => {
		...
		assert.deepStrictEqual(folderChildPos(sf3, 0), ['first', 'last', 'verylast']);
	});
});
