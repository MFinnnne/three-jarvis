...
import {MonitorBinding, MonitorBindingEvents} from '../../../common/binding/monitor';
import {warnMissing} from '../../../common/compat';
import {Emitter} from '../../../common/model/emitter';
import {Value, ValueChangeOptions, ValueEvents} from '../../../common/model/value';
...
function findInputBindingController<In>(bcs: InputBindingController<In>[], b: InputBinding<In>): InputBindingController<In> | null {
...
function findMonitorBindingController<In>(bcs: MonitorBindingController<In>[], b: MonitorBinding<In>): MonitorBindingController<In> | null {
	for (let i = 0; i < bcs.length; i++) {
		const bc = bcs[i];
...
function findValueBladeController<T, V extends View>(bcs: ValueBladeController<T, V>[], v: Value<T>): ValueBladeController<T, V> | null {
	for (let i = 0; i < bcs.length; i++) {
		const bc = bcs[i];
...
function findSubBladeControllerSet(bc: BladeController<View>): NestedOrderedSet<BladeController<View>> | null {
...
export class BladeRack {
	...
	constructor(config: Config) {
		...
		this.blade_?.value('positions').emitter.on('change', this.onBladePositionsChange_);
	...
	private onSetRemove_(ev: NestedOrderedSetEvents<BladeController<View>>['remove']) {
		this.updatePositions_();
	...
	private onChildMonitorUpdate_(ev: MonitorBindingEvents<unknown>['update']): void {
