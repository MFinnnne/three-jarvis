...
export function createEmptyBladeController(doc: Document): BladeController<PlainView> {
...
export class TestValueBladeApi extends BladeApi<LabelController<CheckboxController>> {
	get value(): boolean {
...
export const TestValueBladePlugin: BladePlugin<TestBladeParams> = {
	...
	controller(args) {
		return new LabeledValueController<boolean, CheckboxController>(args.document, {
			blade: createBlade(),
