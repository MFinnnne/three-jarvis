...
export class TabApi extends RackLikeApi<TabController> {
	...
	public on<EventName extends keyof TabApiEvents>(eventName: EventName, handler: (ev: TabApiEvents[EventName]['event']) => void): TabApi {
	...
	private setUpPageApi_(pc: TabPageController) {
		const rackApi = this.rackApi_['apiSet_'].find((api) => api.controller_ === pc.contentController) as RackApi | null;
		if (!rackApi) {
			throw TpError.shouldNeverHappen();
		}
	...
	private onPageRemove_(ev: NestedOrderedSetEvents<TabPageController>['remove']) {
