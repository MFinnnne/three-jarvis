...
describe(TabController.name, () => {
	it('should add page element', () => {
		...
		assert.strictEqual(c.view.itemsElement.contains(pc.itemController.view.element), true);
		assert.strictEqual(c.view.contentsElement.contains(pc.contentController.view.element), true);
	});
	...
	it('should insert page at specific position', () => {
		...
		const pcs = [createTabPage(doc, 'foo'), createTabPage(doc, 'bar'), createTabPage(doc, 'baz')];
		c.add(pcs[0]);
		c.add(pcs[1]);
		c.add(pcs[2], 1);
	...
	it('should remove page', () => {
		...
		assert.strictEqual(c.view.itemsElement.contains(pcs[1].itemController.view.element), false);
		assert.strictEqual(c.view.contentsElement.contains(pcs[1].contentController.view.element), false);
	});
	...
	it('should apply parent disabled', () => {
		...
		assert.strictEqual(pc.itemController.viewProps.globalDisabled.rawValue, true);
	});
});
