...
export class FolderApi extends RackLikeApi<FolderController> implements BladeRackApi {
	...
	constructor(controller: FolderController, pool: PluginPool) {
		...
		this.controller_.foldable.value('expanded').emitter.on('change', (ev: ValueEvents<boolean>['change']) => {
	 ...
	 */
	public on<EventName extends keyof FolderApiEvents>(eventName: EventName, handler: (ev: FolderApiEvents[EventName]['event']) => void): FolderApi {
