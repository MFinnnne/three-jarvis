	...
	[
		...
		context(`when ${JSON.stringify(params)}`, () => {
			it('should initialize buffer', () => {
				assert.deepStrictEqual(initializeBuffer(params.bufferSize).rawValue, expected);
			});
		});
	});
	...
	[
		...
		context(`when ${JSON.stringify(params)}`, () => {
			it('should create pushed buffer', () => {
				assert.deepStrictEqual(createPushedBuffer(params.buffer, params.newValue), expected);
			});
		});
	});
});
