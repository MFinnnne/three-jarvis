...
export function getCanvasContext(canvasElement: HTMLCanvasElement): CanvasRenderingContext2D | null {
...
export function createSvgIconElement(document: Document, iconId: IconId): Element {
	const elem = document.createElementNS(SVG_NS, 'svg');
	elem.innerHTML = ICON_ID_TO_INNER_HTML_MAP[iconId];
	return elem;
}

export function insertElementAt(parentElement: Element, element: Element, index: number): void {
