	...
	[
		...
		context(`when ${JSON.stringify(params)}`, () => {
			it('should parse array parameter', () => {
				assert.deepStrictEqual(ParamsParsers.required.array(params.parser)(params.value), expected);
			});
		});
	});
	...
	[
		...
		context(`when ${JSON.stringify(params)}`, () => {
			it('should parse object parameter', () => {
				assert.deepStrictEqual(ParamsParsers.required.object(params.parser)(params.value), expected);
			});
		});
	});
});
