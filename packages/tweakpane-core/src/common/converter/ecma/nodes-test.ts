...
describe(BinaryOperationNode.name, () => {
	it('should throw error for unexpected operator', () => {
		assert.throws(() => {
			new BinaryOperationNode('@', new NumberLiteralNode('1'), new NumberLiteralNode('2')).evaluate();
		});
	});
	it('should convert into string', () => {
		const op = new BinaryOperationNode('**', new NumberLiteralNode('1'), new NumberLiteralNode('2'));
