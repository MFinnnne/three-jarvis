...
describe(MonitorBinding.name, () => {
	...
	it('should bind value', () => {
		...
		assert.strictEqual(value.rawValue[0], 123, 'Initial value of target should be applied');

		obj.foo = 456;
		ticker.tick();

		assert.strictEqual(value.rawValue[0], 456, 'Binded value should be updated');
	});
	...
	it('should bind value', () => {
		...
		assert.strictEqual(value.rawValue[0], 123, 'Deleted value should not be pushed');
	});
});
