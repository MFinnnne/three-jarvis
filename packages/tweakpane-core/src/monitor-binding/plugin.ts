...
function createTicker(document: Document, interval: number | undefined): Ticker {
	return interval === 0 ? new ManualTicker() : new IntervalTicker(document, interval ?? Constants.monitor.defaultInterval);
}
...
export function createMonitorBindingController<T, P extends BaseMonitorParams>(
	...
	const bufferSize = P.optional.number(args.params.bufferSize).value ?? (plugin.binding.defaultBufferSize && plugin.binding.defaultBufferSize(result.params)) ?? 1;
