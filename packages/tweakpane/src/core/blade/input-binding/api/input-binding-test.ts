...
describe(InputBindingApi.name, () => {
	...
	it('should update and listen change event', (done) => {
		const PARMAS = {
			foo: 0,
		};
		const api = createApi(new BindingTarget(PARMAS, 'foo'));
		api.setValue(123, true);
		api.on('change', (ev) => {
			assert.strictEqual(ev instanceof TpChangeEvent, true);
			assert.strictEqual(ev.target, api);
			assert.strictEqual(ev.presetKey, 'foo');
	...
	it('should update and not listen change event', (done) => {
		const PARMAS = {
			foo: 0,
		};
		const api = createApi(new BindingTarget(PARMAS, 'foo'));
		...
		api.setValue(123, false);
		const value = api.controller_.binding.value.rawValue;
